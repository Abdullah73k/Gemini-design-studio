...
function buildRoomDesignerPrompt(args: {
  ...
  return [
    ...
    "- Orientation: For items placed against a wall, rotate so the item's \"front\" faces toward the room center. Keep the long axis (max(w,d)) parallel to the wall it sits against.",
    "- Orientation examples: a desk against a wall should face inward; the chair should be centered on the desk's normal (behind the desk) with ~0.6–0.7m spacing; wardrobes face outward into the room.",
    "- Rotation output: prefer only rotation_deg.y; keep rotation_deg.x = 0 and rotation_deg.z = 0 unless explicitly asked (e.g., tilted posters).",
    "- AABB spacing: Treat each object as a box centered at position_m with half-extents (w/2, d/2). Ensure boxes do not intersect; maintain ≥0.2m clearance where possible.",
    "",
